  ...
  prims_mst(start_node) {
    ...
    for (let i = 0; i < this.size - 1; i++) {
      ...
      for (let edge of this.adj[u].values()) {
        ...
        if (
          processed[edge[0]] == false &&
          values[u] + edge[1] < values[edge[0]]
        ) {
          ...
          parent[edge[0]] = u;
          //now update the values array to the weight of this current edge plus the weight of the parent where it came from
          values[edge[0]] = values[u] + edge[1];
